...
`;var d={appShell:document.querySelector(`.app-shell`),navItems:document.querySelectorAll(`.side-nav-item`),sectionTitle:document.querySelector(`#sectionTitle`),sampleSection:document.querySelector(`.sample-section`),sampleSelect:document.querySelector(`#sampleSelect`),sampleToggle:document.querySelector(`#sampleToggle`),sampleGrid:document.querySelector(`#sampleGrid`),fileInput:document.querySelector(`#fileInput`),renderButton:document.querySelector(`#renderButton`),clearButton:document.querySelector(`#clearButton`),diagnosticsToggle:document.querySelector(`#diagnosticsToggle`),diagnosticsOverlay:document.querySelector(`#diagnosticsOverlay`),diagnosticsDrawer:document.querySelector(`#diagnosticsDrawer`),diagnosticsClose:document.querySelector(`#diagnosticsClose`),previewInfoToggle:document.querySelector(`#previewInfoToggle`),previewCloseButton:document.querySelector(`#previewCloseButton`),formatBadge:document.querySelector(`#formatBadge`),fileTitle:document.querySelector(`#fileTitle`),fileDetail:document.querySelector(`#fileDetail`),renderStatus:document.querySelector(`#renderStatus`),timeStatus:document.querySelector(`#timeStatus`),modelStatus:document.querySelector(`#modelStatus`),wasmStatus:document.querySelector(`#wasmStatus`),byteStatus:document.querySelector(`#byteStatus`),previewCard:document.querySelector(`.preview-card`),previewStage:document.querySelector(`.preview-stage`),preview:document.querySelector(`#preview`),diagnostics:document.querySelector(`#diagnostics`)};f();function f(){H(),U(),ee(),se(),x(X());let e=Fe();if(e?.error){k();let t=e.error instanceof Error?e.error.message:String(e.error);B(`失败`,`0 ms`,`-`,`参数错误`),d.diagnostics.textContent=t,R(t);return}if(e){ne(e);return}p(l.selectedId,!1,{focusPreview:!1})}function ee(){for(let e of d.navItems)e.addEventListener(`click`,()=>{Se(e.dataset.category||`all`)});d.sampleSelect.addEventListener(`change`,()=>{d.sampleSelect.value.startsWith(`__`)||p(d.sampleSelect.value,E())}),d.sampleToggle.addEventListener(`click`,()=>{A(!l.samplesCollapsed)}),d.previewInfoToggle.addEventListener(`click`,()=>{T(!l.previewInfoOpen)}),d.previewCloseButton.addEventListener(`click`,()=>{ve()}),d.sampleGrid.addEventListener(`click`,e=>{let t=e.target.closest(`.sample-card`);t&&p(t.dataset.sampleId,!0)}),d.renderButton.addEventListener(`click`,()=>{m()}),d.diagnosticsToggle.addEventListener(`click`,()=>{q(!l.diagnosticsOpen)}),d.diagnosticsClose.addEventListener(`click`,()=>{q(!1)}),d.diagnosticsOverlay.addEventListener(`click`,()=>{q(!1)}),d.clearButton.addEventListener(`click`,()=>{l.currentBytes=null,l.externalSource=null,x(!1),k(),B(`未渲染`,`0 ms`,`-`,`未加载`),d.byteStatus.textContent=`0 KB`,d.diagnostics.textContent=`请选择样例或上传文件，然后点击“渲染当前文件”。`,W()}),d.fileInput.addEventListener(`change`,()=>{te()})}async function te(){let e=++l.requestToken;try{let t=d.fileInput.files?.[0];if(!t)return;let n=Ae(t.name),r=K(n);if(l.selectedId=`__upload__`,l.activeCategory=r.category,l.externalSource=null,x(!1),V(),l.currentTitle=t.name,l.currentFormat=n,H(),U(),b({format:n,title:t.name,detail:`正在读取本地文件`}),j(`正在读取本地文件...`,n),B(`读取上传`,`0 ms`,`-`,`准备渲染`),d.byteStatus.textContent=`0 KB`,d.diagnostics.textContent=`文件已选择，正在读取并准备自动渲染。`,W(),await L(),l.currentBytes=await t.arrayBuffer(),e!==l.requestToken)return;b({format:n,title:t.name,detail:`上传文件，${$(l.currentBytes.byteLength)}`}),d.byteStatus.textContent=$(l.currentBytes.byteLength),F(`文件读取完成，正在启动预览引擎...`,n),d.diagnostics.textContent=`文件已读取，正在自动渲染。`,await m()}catch(e){z(e)}finally{d.fileInput.value=``}}async function p(e,t,n={}){let r=i.find(t=>t.id===e)??i[0];l.selectedId=r.id,l.externalSource=null,x(!1),l.currentBytes=null,l.currentTitle=r.title,l.currentFormat=r.format,d.sampleSelect.value=r.id,b(r),W(),k(),B(r.path?`已选择`:`等待上传`,`0 ms`,r.path?`-`:`需要 .ppt 文件`,`未加载`),d.byteStatus.textContent=`0 KB`,d.diagnostics.textContent=r.path?`样例已选择，点击“渲染当前文件”开始检查。`:`当前没有内置 .ppt 样例，请上传 .ppt 文件检查。`,t?await m():n.focusPreview!==!1&&D()}async function ne(e){l.selectedId=`__external__`,l.externalSource=e,x(!0),l.currentBytes=null,l.currentTitle=e.name,l.currentFormat=e.format,S(`__external__`,`可道云文件 - ${e.name}`),d.sampleSelect.value=`__external__`,b({format:e.format,title:e.name,detail:e.detail}),W(),j(`正在读取文档...`,e.format),B(`读取链接`,`0 ms`,`-`,`未加载`),d.byteStatus.textContent=`0 KB`,d.diagnostics.textContent=`检测到可道云传入的同源文件链接，正在读取并渲染。`,await m()}async function m(){let e=++l.requestToken,t=i.find(e=>e.id===l.selectedId);if(!l.currentBytes)if(l.externalSource){j(`正在下载服务器文件...`,l.externalSource.format),B(`读取链接`,`0 ms`,`-`,`未加载`);try{l.currentBytes=await h(l.externalSource.url,{credentials:`include`,cache:`no-store`,format:l.externalSource.format,label:`服务器文件`})}catch(e){z(e);return}d.byteStatus.textContent=$(l.currentBytes.byteLength)}else if(t?.path)j(`正在下载样例文件...`,t.format),B(`读取文件`,`0 ms`,`-`,`未加载`),await L(),l.currentBytes=await h(t.path,{credentials:`same-origin`,format:t.format,label:`样例文件`}),l.currentTitle=t.title,l.currentFormat=t.format,d.byteStatus.textContent=$(l.currentBytes.byteLength);else{B(`等待上传`,`0 ms`,`需要 .ppt 文件`,`未加载`);return}if(e!==l.requestToken)return;if(Ne(l.currentTitle,l.currentBytes)){z(Error(`文件为空或为 Office 临时锁文件（~$），无法预览。请上传实际 Office 文件。`));return}let n=Pe(l.currentFormat,l.currentBytes);if(n!==l.currentFormat){let e=l.currentFormat;l.currentFormat=n,b({format:n,title:l.currentTitle,detail:`文件内容识别为 ${n.toUpperCase()}（原扩展名 .${e}）`})}C(l.currentFormat),M(`正在准备预览环境...`,l.currentFormat),B(`按需加载`,`0 ms`,`-`,`准备 WASM`),d.renderButton.disabled=!0,await L();try{let t=performance.now(),n=await ge(l.currentFormat,l.currentBytes);if(e!==l.requestToken)return;if(!n.ok){let e=n.error?.message||`WASM 安全校验未通过。`,t=n.error?.code||`wasm-check-failed`;throw Error(`WASM 安全校验未通过：${t}。${e}`)}let r=await ae(l.currentFormat,l.currentBytes,e);if(e!==l.requestToken)return;_();let i=Math.round(performance.now()-t),a=n.ok?`正常`:`需检查`;B(`完成`,`${i} ms`,r.summary,a),d.diagnostics.textContent=JSON.stringify({文件:l.currentTitle,格式:l.currentFormat,字节:l.currentBytes.byteLength,渲染:r,WASM:n},null,2)}catch(e){z(e)}finally{d.renderButton.disabled=!1}}async function h(e,t={}){let{format:n=l.currentFormat,label:r=`服务器文件`,...i}=t;F(`正在下载${r}...`,n),I({received:0,total:0,label:r});let a=await fetch(e,i);if(!a.ok)throw Error(`${r}读取失败：${a.status} ${a.statusText}`);let o=re(a.headers.get(`content-length`));if(!a.body||typeof a.body.getReader!=`function`){let e=await a.arrayBuffer();return I({received:e.byteLength,total:e.byteLength,label:r}),d.byteStatus.textContent=$(e.byteLength),new Uint8Array(e)}let s=a.body.getReader(),c=[],u=0;for(;;){let{value:e,done:t}=await s.read();if(t)break;e&&(c.push(e),u+=e.byteLength,d.byteStatus.textContent=o?`${$(u)} / ${$(o)}`:$(u),I({received:u,total:o,label:r}),F(o?`正在下载${r}，${Math.min(99,Math.floor(u/o*100))}%`:`正在下载${r}，已接收 ${$(u)}`,n))}let f=ie(c,u);return I({received:f.byteLength,total:o||f.byteLength,label:r}),F(`${r}下载完成，正在启动预览引擎...`,n),d.byteStatus.textContent=$(f.byteLength),f}function re(e){if(!e)return 0;let t=Number(e);return Number.isFinite(t)&&t>0?t:0}function ie(e,t){let n=new Uint8Array(t),r=0;for(let t of e)n.set(t,r),r+=t.byteLength;return n}async function ae(e,t,n){if(e===`doc`){B(`加载 DOC 渲染器`,`0 ms`,`-`,`正常`),await P(`正在加载 Word 渲染器...`,e);let{parseMsDoc:n,renderMsDoc:r,mountMsDoc:i,convertMetafileToSvg:a}=await y(`doc`);B(`解析 DOC`,`0 ms`,`-`,`正常`),await P(`正在解析 Word 二进制文档...`,e);let o=n(t,{});B(`渲染 DOC`,`0 ms`,`-`,`正常`),await P(`正在生成 Word 页面...`,e);let s=await pe(o,a),c=r(o);O(),i(d.preview,c),l.activeViewerHandle={destroy(){d.preview.__msdocCleanup?.(),delete d.preview.__msdocCleanup}};let u=d.preview.querySelectorAll(`.msdoc-page`).length;return{summary:u?`${u} 页`:`${o.blocks.length} 个块`,warnings:c.warnings.length,blocks:o.blocks.length,assets:o.assets.length,pages:u,vectorAssets:s}}if(e===`docx`){B(`加载 DOCX 渲染器`,`0 ms`,`-`,`正常`),await P(`正在加载 Word 渲染器...`,e);let{renderAsync:n}=await y(`docx`);B(`渲染 DOCX`,`0 ms`,`-`,`正常`),await P(`正在解析并排版 Word 文档...`,e),await n(new Blob([t]),d.preview,d.preview,{inWrapper:!0,breakPages:!0,ignoreWidth:!1,ignoreHeight:!1,strictWordCompatibility:!0,awaitLayout:!0,paginationTolerance:2,maxDynamicPaginationPasses:1e3});let r=d.preview.querySelectorAll(`section.docx`).length,i=d.preview.querySelectorAll(`img`).length,a=d.preview.querySelectorAll(`table`).length;return{summary:`${r||1} 页`,sections:r||1,tables:a,images:i}}if(e===`ppt`){B(`加载 PPT 渲染器`,`0 ms`,`-`,`正常`),await P(`正在加载 PowerPoint 渲染器...`,e);let{parsePptBinary:r,renderSlideToHtmlAsync:i,defaultPptViewerCss:a}=await y(`ppt`);B(`解析 PPT`,`0 ms`,`-`,`正常`),await P(`正在解析 PowerPoint 二进制内容...`,e);let o=r(t,{});return B(`渲染 PPT`,`0 ms`,`-`,`正常`),await P(`正在生成首屏幻灯片...`,e),await oe(o,{renderSlideToHtmlAsync:i,defaultPptViewerCss:a,token:n}),{summary:`${o.slides.length} 页`,slides:o.slides.length,masters:o.masters?.length??0}}if(e===`pptx`){B(`加载 PPTX 渲染器`,`0 ms`,`-`,`正常`),await P(`正在加载 PowerPoint 渲染器...`,e);let{parsePptx:n,renderPresentationToElement:r}=await y(`pptx`);B(`解析 PPTX`,`0 ms`,`-`,`正常`),await P(`正在解析 PowerPoint Open XML 内容...`,e);let i=await n(t,{onProgress(t){d.renderStatus.textContent=t.stage,F(t.stage||`正在解析 PowerPoint 内容...`,e)}});return B(`渲染 PPTX`,`0 ms`,`-`,`正常`),await P(`正在建立按需渲染画布...`,e),O(),d.preview.innerHTML=``,l.activeViewerHandle=r(i,d.preview,{uiMode:`bare`,virtualize:!0,showNotes:!1,rootMargin:`1200px 0px`}),await L(),_(),{summary:`${i.presentation.slides.length} 页`,slides:i.presentation.slides.length,masters:i.presentation.slideMasters.length,layouts:i.presentation.slideLayouts.length,warnings:i.warnings.length}}if(J(e)){B(`加载 Excel 渲染器`,`0 ms`,`-`,`正常`),await P(`正在加载 Excel 渲染器...`,e);let{createExcelWorkerClient:n,mountExcel:r}=await y(`excel`);B(`解析 Excel`,`0 ms`,`-`,`正常`),await P(`正在启动 Excel Worker...`,e);let i=n({worker:await fe()}),a=t=>ye(t,e);try{let n=await i.load(t,{format:e,onProgress:a});B(`渲染 Excel`,`0 ms`,`-`,`正常`),await P(`正在生成表格预览...`,e);let o=r(d.preview,n,{workerClient:i,onProgress:a,terminateWorkerOnDestroy:!0});l.activeViewerHandle=o;let s=n.sheets.find(e=>e.id===n.activeSheetId)||n.sheets[0];return{summary:o.summary,sheets:n.sheets.length,activeSheet:s?.name,rows:s?.rowCount??0,columns:s?.colCount??0,engine:n.engine,virtualSheets:n.sheets.filter(e=>e.virtualWindowed).length}}catch(e){throw i.terminate?.(),e}}throw Error(`不支持的格式：${e}`)}async function oe(e,t){let{renderSlideToHtmlAsync:n,defaultPptViewerCss:r,token:i}=t,a=e.slides.length;O(),d.preview.innerHTML=`
    <style>${r()}</style>
  ...
  `;let o=d.preview.querySelector(`.progressive-render-root`),s=d.preview.querySelector(`.progressive-render-status`);if(o){for(let t=0;t<a;t+=1){if(i!==l.requestToken)return;let r=e.slides[t],c=t+1,u=`正在渲染第 ${c} / ${a} 页`;B(`渲染 PPT`,`0 ms`,`${c}/${a}`,`正常`),s&&(s.textContent=u);let d=await n(r,e);r.html=d,o.insertAdjacentHTML(`beforeend`,d),_(),await L()}s&&(s.textContent=`已完成 ${a} 页渲染`,window.setTimeout(()=>s.remove(),1200)),e.html=d.preview.innerHTML}}function se(){typeof ResizeObserver<`u`?new ResizeObserver(()=>g()).observe(d.preview):window.addEventListener(`resize`,()=>g())}function g(){l.layoutFrame||=requestAnimationFrame(()=>{l.layoutFrame=0,_()})}function _(){if(l.currentFormat===`doc`){v(`.preview-doc .msdoc-page`,`word-page-frame`);return}if(l.currentFormat===`docx`){v(`.preview-docx section.docx`,`word-page-frame`);return}if(l.currentFormat===`ppt`||l.currentFormat===`pptx`){if(l.currentFormat===`pptx`&&d.preview.querySelector(`.pptxv-root [data-slide-index]`)){v(`.pptxv-root [data-slide-index]`,`slide-page-frame`);return}v(`.pptxv-slide, .ppt-viewer-slide`,`slide-page-frame`)}}function v(e,t){let n=Array.from(d.preview.querySelectorAll(e));if(!n.length)return;let r=le();for(let e of n){let n=ce(e,t);e.classList.add(`scaled-preview-page`),e.style.transform=`none`,e.style.transformOrigin=`top left`,e.style.margin=`0`,e.style.maxWidth=`none`;let i=ue(e);e.style.width=`${i}px`,e.style.minWidth=`${i}px`;let a=Math.max(1,e.scrollHeight,e.offsetHeight,e.getBoundingClientRect().height),o=Math.min(1,r/i);n.style.width=`${Math.ceil(i*o)}px`,n.style.height=`${Math.ceil(a*o)}px`,n.style.setProperty(`--preview-page-scale`,o.toFixed(5)),e.style.transform=`scale(${o})`}}function ce(e,t){let n=e.parentElement;if(n?.classList.contains(t))return n;let r=document.createElement(`div`);return r.className=t,n?.insertBefore(r,e),r.appendChild(e),r}function le(){let e=getComputedStyle(d.preview),t=parseFloat(e.paddingLeft||`0`)+parseFloat(e.paddingRight||`0`);return Math.max(280,d.preview.clientWidth-t)}function ue(e){if(e.classList.contains(`word-page`))return 794;let t=de(e.style.width);if(Number.isFinite(t)&&t>0)return t;let n=de(getComputedStyle(e).width);return Number.isFinite(n)&&n>0?n:Math.max(1,e.scrollWidth,e.offsetWidth,e.getBoundingClientRect().width)}function de(e){let t=String(e||``).trim().match(/^(-?\d+(?:\.\d+)?)(px|pt|in|cm|mm|pc|q)?$/i);if(!t)return NaN;let n=Number(t[1]);if(!Number.isFinite(n))return NaN;let r=(t[2]||`px`).toLowerCase();return r===`px`?n:r===`pt`?96/72*n:r===`in`?n*96:r===`cm`?96/2.54*n:r===`mm`?96/25.4*n:r===`pc`?n*16:r===`q`?96/101.6*n:NaN}async function y(e){if(!l.modules.has(e)){let t=r(e===`doc`?()=>import(`./BTREDByE.js`):e===`docx`?()=>import(`./pgL_B536.js`):e===`ppt`?()=>import(`./hrRjU6Vj.js`):e===`pptx`?()=>import(`./D28lHkZG.js`):()=>import(`./Cs_1smMj.js`),[],import.meta.url);l.modules.set(e,t)}return l.modules.get(e)}async function fe(){let{default:e}=await r(async()=>{let{default:e}=await import(`./Dg3QxHNX.js`);return{default:e}},[],import.meta.url);return new e}async function pe(e,t){let n={scanned:0,converted:0,failed:0},r=(e.assets||[]).filter(e=>e?.type===`image`&&/^image\/(?:emf|wmf)$/i.test(e.mime));for(let e of r){n.scanned+=1;try{let r=await me(e),i=r?t(e.mime,r):null;if(!i){n.failed+=1;continue}e.mime=i.mime,e.bytes=i.bytes,e.dataUrl=i.dataUrl,e.displayable=!0,e.meta={...e.meta||{},vectorConverted:!0,vectorSourceMime:i.sourceMime,vectorWidth:i.width,vectorHeight:i.height,vectorRecordCount:i.recordCount},n.converted+=1}catch{n.failed+=1}}return n}async function me(e){let t=e.bytes instanceof Uint8Array?e.bytes:new Uint8Array(e.bytes||[]);return t.length?e.meta?.metafileCompressed&&e.meta.metafileCompression===0&&e.meta.metafileFilter===254?he(t):t:null}async function he(e){if(typeof DecompressionStream>`u`)throw Error(`当前浏览器不支持 DecompressionStream，无法解压 DOC 内嵌矢量图。`);let t=new Blob([e.slice()]).stream().pipeThrough(new DecompressionStream(`deflate`));return new Uint8Array(await new Response(t).arrayBuffer())}async function ge(e,t){B(`加载 WASM`,`0 ms`,`-`,`加载中`),await P(`正在加载 WASM 安全校验...`,e),l.wasmRuntimePromise||=r(()=>import(`./DxnJeu1w.js`),[],import.meta.url);let{loadOfficeWasmCore:n}=await l.wasmRuntimePromise;l.wasmCorePromise||=n(`wasm/office-parser-core.wasm`);let i=await l.wasmCorePromise;if(!i.authorization.ok)return _e(i.authorization);if(e===`xls`)return{ok:!0,engine:`assemblyscript-wasm`,parser:`origin-access-control`,wasm:!0,access:i.authorization.access,documentParser:`xls-binary-js-parser`,note:`XLS 二进制解析当前由 excel-viewer 的 hucre 路径处理，渲染前仍必须通过 WASM 域名授权。`};let a=e=>({..._e(e),access:i.authorization.access});return e===`doc`?a(i.parseMsdoc(t)):e===`ppt`?a(i.parsePpt(t)):e===`docx`?a(i.parseDocx(t)):e===`xlsx`||e===`xlsb`?a(i.parseXlsx(t)):e===`pptx`?a(i.parsePptx(t)):{ok:!1,error:`不支持的格式：${e}`}}function _e(e){return!e||e.error||e.parser===`origin-access-control`?{ok:!!e?.ok,engine:e?.engine??`assemblyscript-wasm`,parser:e?.parser??`unknown`,access:e?.access,error:e?.error}:e.container?{ok:e.ok,engine:e.engine,parser:e.parser,entries:e.container.entries.length,wordDocument:e.wordDocument??void 0,records:e.records??void 0}:{ok:e.ok,engine:e.engine,parser:e.parser,entryCount:e.package.entryCount,mainPart:e.package.mainPart,hasMainPart:e.package.hasMainPart,hasContentTypes:e.package.hasContentTypes}}function b(e){let t=K(e.format);Oe(e.format),d.formatBadge.textContent=t.label,d.formatBadge.className=`format-badge tone-${t.tone}`,d.fileTitle.textContent=e.title,d.fileDetail.textContent=e.detail}function x(e){l.embedMode=e,d.appShell.classList.toggle(`embed-mode`,e),document.body.classList.toggle(`embed-mode`,e),e&&q(!1)}function S(e,t){let n=d.sampleSelect.querySelector(`option[value="${e}"]`);n||(n=document.createElement(`option`),n.value=e,d.sampleSelect.prepend(n)),n.textContent=t}function C(e){let t=e===`doc`||e===`docx`?`preview-word`:J(e)?`preview-excel`:`preview-slides`;w(!0),A(!0),d.preview.className=`preview preview-${e} ${t}`,d.previewStage.dataset.previewMode=t,D()}function w(e){d.appShell.classList.toggle(`preview-active`,!!e),e||T(!1)}function T(e){l.previewInfoOpen=!!e,d.appShell.classList.toggle(`preview-info-open`,l.previewInfoOpen),d.previewInfoToggle.setAttribute(`aria-expanded`,String(l.previewInfoOpen)),d.previewInfoToggle.setAttribute(`aria-label`,l.previewInfoOpen?`隐藏预览信息`:`显示预览信息`)}function E(){return window.matchMedia?.(`(max-width: 760px)`).matches??window.innerWidth<=760}function D(){!E()||l.embedMode||requestAnimationFrame(()=>{if(d.appShell.classList.contains(`preview-active`)){window.scrollTo({top:0,left:0,behavior:`auto`});return}d.previewCard?.scrollIntoView({block:`start`,behavior:`smooth`})})}function ve(){l.requestToken+=1,k();let e=i.find(e=>e.id===l.selectedId);B(l.currentBytes||l.externalSource||e?.path?`已选择`:`等待上传`,`0 ms`,l.currentBytes?`已缓存`:`-`,`未加载`),d.diagnostics.textContent=`已关闭预览，可重新选择样例或再次渲染当前文件。`,!(!E()||l.embedMode)&&requestAnimationFrame(()=>{d.sampleSection?.scrollIntoView({block:`start`,behavior:`smooth`})})}function O(){if(l.activeViewerHandle){try{l.activeViewerHandle.destroy?.()}catch{}l.activeViewerHandle=null}}function k(){O(),w(!1),A(!1),delete d.previewStage.dataset.previewMode,d.preview.className=`preview preview-empty`,d.preview.innerHTML=``}function A(e){l.samplesCollapsed=!!e;let t=!l.samplesCollapsed;d.appShell.classList.toggle(`samples-collapsed`,l.samplesCollapsed),d.sampleToggle.textContent=t?`收起样例`:`展开样例`,d.sampleToggle.setAttribute(`aria-expanded`,String(t))}function j(e,t=l.currentFormat){t?C(t):d.preview.className=`preview preview-loading`,M(e,t)}function M(e,t=l.currentFormat){O(),d.preview.innerHTML=N(e,t)}function N(e,t=l.currentFormat){let n=K(t),r=xe(t);return`
  ...
  `}async function P(e,t=l.currentFormat){F(e,t),await L()}function F(e,t=l.currentFormat){let n=d.preview.querySelector(`.loading-message`);if(!n){t&&C(t),M(e,t);return}n.textContent=e}function I(e){let t=d.preview.querySelector(`.loading-progress`);if(!t)return;let n=t.querySelector(`.loading-progress-bar`),r=t.querySelector(`.loading-progress-text`),i=Number.isFinite(e?.percent)?Math.max(0,Math.min(100,Math.round(e.percent))):null;if(i!==null){t.hidden=!1,n.style.width=`${i}%`,n.classList.remove(`is-indeterminate`),r.textContent=`${i}%`;return}let a=Math.max(0,e?.received??0),o=Math.max(0,e?.total??0),s=a>0||o>0;if(t.hidden=!s,s){if(o>0){let e=Math.max(0,Math.min(100,Math.round(a/o*100)));n.style.width=`${e}%`,n.classList.remove(`is-indeterminate`),r.textContent=`${e}% · ${$(a)} / ${$(o)}`;return}n.style.width=`42%`,n.classList.add(`is-indeterminate`),r.textContent=`已下载 ${$(a)}`}}function ye(e,t=l.currentFormat){let n=e?.message||be(e?.stage);d.renderStatus.textContent=n,F(n,t),Number.isFinite(e?.progress)&&I({percent:e.progress*100})}function be(e){return e===`received`?`Excel Worker 已接收文件`:e===`parsing`?`Excel Worker 正在解析工作簿`:e===`normalizing`?`正在整理工作表结构`:e===`ready`?`工作簿已准备完成`:e===`window`?`正在按需加载表格窗口`:`正在处理 Excel 工作簿`}function xe(e){let t=K(e);return t.theme===`excel`?`Excel 正在加载`:t.theme===`powerpoint`?`PowerPoint 正在加载`:`Word 正在加载`}function L(){return new Promise(e=>{if(typeof requestAnimationFrame!=`function`){setTimeout(e,0);return}requestAnimationFrame(()=>requestAnimationFrame(e))})}function R(e){d.preview.className=`preview preview-message`,d.preview.textContent=e}function z(e){let t=e instanceof Error?e.message:String(e);k(),l.embedMode&&R(t),B(`失败`,`0 ms`,`-`,t.includes(`WASM 安全校验未通过`)?`已拦截`:t.includes(`Office 临时锁文件`)||t.includes(`文件为空`)?`已跳过`:`异常`),d.diagnostics.textContent=e instanceof Error?e.stack||e.message:String(e)}function B(e,t,n,r){d.renderStatus.textContent=e,d.timeStatus.textContent=t,d.modelStatus.textContent=n,d.wasmStatus.textContent=r}function Se(e){if(!o[e])return;l.activeCategory=e,V(),H(),U();let t=i.find(e=>e.id===l.selectedId);(!t||!Ce(t))&&p((G()[0]||i[0]).id,!1,{focusPreview:!1})}function V(){for(let e of d.navItems){let t=e.dataset.category===l.activeCategory;e.classList.toggle(`active`,t),t?e.setAttribute(`aria-current`,`page`):e.removeAttribute(`aria-current`)}d.sectionTitle.textContent=o[l.activeCategory]?.sampleTitle||o.all.sampleTitle}function H(){let e=G();if(d.sampleSelect.innerHTML=e.map(e=>`<option value="${Q(e.id)}">${Z(e.title)} - ${Z(e.detail)}</option>`).join(``),l.selectedId===`__external__`){S(`__external__`,`可道云文件 - ${l.currentTitle||`外部文件`}`),d.sampleSelect.value=`__external__`;return}if(l.selectedId===`__upload__`){S(`__upload__`,`上传文件 - ${l.currentTitle||`本地文件`}`),d.sampleSelect.value=`__upload__`;return}let t=e.some(e=>e.id===l.selectedId)?l.selectedId:e[0]?.id;t&&(d.sampleSelect.value=t)}function U(){d.sampleGrid.innerHTML=G().map(e=>{let t=K(e.format);return`
    ...
    `}).join(``),W()}function W(){let e=d.sampleGrid.querySelectorAll(`.sample-card`);for(let t of e){let e=t.dataset.sampleId===l.selectedId;t.classList.toggle(`active`,e),t.setAttribute(`aria-pressed`,String(e))}}function G(){return l.activeCategory===`all`?i:i.filter(e=>Ce(e))}function Ce(e){return l.activeCategory===`all`?!0:K(e.format).category===l.activeCategory}function we(e,t){let n=J(e.format)?`thumb-sheet`:e.format===`ppt`||e.format===`pptx`?`thumb-slide`:`thumb-doc`;return`
  ...
  `}function De(){return Array.from({length:24},()=>`<span></span>`).join(``)}function K(e){return a[e]||{label:String(e||`FILE`).toUpperCase(),icon:`F`,group:`文件`,tone:`blue`,category:`document`,theme:`word`}}function q(e){l.diagnosticsOpen=e,d.appShell.classList.toggle(`diagnostics-open`,e),d.diagnosticsToggle.setAttribute(`aria-expanded`,String(e)),d.diagnosticsDrawer.setAttribute(`aria-hidden`,String(!e)),d.diagnosticsOverlay.hidden=!e}function Oe(e){let t=ke(e);document.body.dataset.theme=t,d.appShell.dataset.theme=t}function ke(e){return K(e).theme}function Ae(e){let t=e.split(`.`).pop()?.toLowerCase();if(Me(t))return t;throw Error(`不支持的文件扩展名：${e}`)}function je(e){let t=String(e||``).replace(/^\./,``).toLowerCase();if(Me(t))return t;throw Error(`不支持的文件格式：${e}`)}function Me(e){return[`doc`,`docx`,`ppt`,`pptx`,`xls`,`xlsx`,`xlsb`].includes(e)}function J(e){return e===`xls`||e===`xlsx`||e===`xlsb`}function Ne(e,t){return!t||t.byteLength===0?!0:(String(e||``).split(/[\\/]/).pop()||``).startsWith(`~$`)}function Pe(e,t){let n=t instanceof Uint8Array?t:new Uint8Array(t||[]);if(n.length<4)return e;if(n[0]===80&&n[1]===75){if(Y(n,`word/document.xml`))return`docx`;if(Y(n,`ppt/presentation.xml`))return`pptx`;if(Y(n,`xl/workbook.xml`))return`xlsx`}return e}function Y(e,t){let n=Array.from(t,e=>e.charCodeAt(0));if(!n.length||e.length<n.length)return!1;let r=n[0],i=e.length-n.length;for(let t=0;t<=i;t+=1){if(e[t]!==r)continue;let i=!0;for(let r=1;r<n.length;r+=1)if(e[t+r]!==n[r]){i=!1;break}if(i)return!0}return!1}function Fe(){let e=new URLSearchParams(window.location.search),t=e.get(`file`)||e.get(`src`)||e.get(`url`);if(!t)return null;try{let n=new URL(t,window.location.href);if(n.origin!==window.location.origin)throw Error(`为了离线部署安全，预览文件链接必须与查看器同源。`);let r=e.get(`name`)||Ie(n)||`KodCloud Office 文件`,i=e.get(`type`)?je(e.get(`type`)):Ae(r);return{url:n.href,name:r,format:i,detail:`可道云传入文件，同源读取`}}catch(e){return{error:e}}}function X(){let e=new URLSearchParams(window.location.search);return e.has(`file`)||e.has(`src`)||e.has(`url`)}function Ie(e){let t=decodeURIComponent(e.pathname.split(`/`).filter(Boolean).pop()||``);return e.searchParams.get(`name`)||e.searchParams.get(`filename`)||t}function Z(e){return String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}function Q(e){return Z(e)}function $(e){return e<1024?`${e} B`:e<1024*1024?`${Math.round(e/1024)} KB`:`${(e/1024/1024).toFixed(1)} MB`}